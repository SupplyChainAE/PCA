...
public class MappingController {
	...
	public String parametersPrice( ModelMap map){
		...
		System.out.println("pr"+prList.get(0).getPrice());
	...
	public String savePriority(@RequestParam("id") Long[] idList,@RequestParam("price") Double[] price, ModelMap map){
	...
	public void downloadPincode(ModelMap map,HttpServletResponse response) throws ParseException{
		try {
			String line="Pincode";
			response.setContentType("text/csv");
			response.setHeader("Content-Disposition", "attachment; filename=PincodeTemplate.csv");
	...
	public String updatePincode(@ModelAttribute("postedFile") FileBean file,@ModelAttribute("action") String action, @ModelAttribute("warehouse") Long whId,ModelMap map) throws ParseException{
		try {
	...
	public String updateCourierPincode(@ModelAttribute("postedFile") FileBean file,@RequestParam("action") String action, @RequestParam("courier") Long crId ,  ModelMap map){
	...
	public String updateAwbRequest(@ModelAttribute("postedFile") FileBean file,@RequestParam("action") String action, @RequestParam("courier") Long crId ,  ModelMap map){
			...
			if(postedFile != null)
			{
				InputStream inputStream;
					...
					while ((line = bufferedReader.readLine()) != null)
					{
						...
						if (header==1){
							if (!line.trim().equals("Awb No") || !line.trim().equals("Reference Number")){
								error = 1;
							}
							header = 0;
						}
							...
							if (line.trim().length() > 2){
								if (action.equals("Add")){
									...
									localDao.insertAwb(crId,columns[0],userId,columns[1]);
									error = 2;
								}
								else{
		...
		public void downloadCourier(ModelMap map,HttpServletResponse response) throws ParseException{
			try {
				String line="Awb No,Reference Number";
				response.setContentType("text/csv");
				response.setHeader("Content-Disposition", "attachment; filename=AwbTemplate.csv");
				response.setContentLength(line.length());
				response.getWriter().write(line);
			} catch (IOException e) {
				e.printStackTrace();
